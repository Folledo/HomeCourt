...
class NewMessageController: UITableViewController {
	...
	func fetchUser() {
		...
		Database.database().reference().child("users").observe(.childAdded, with: { (snapshot) in
			...
			if let dictionary = snapshot.value as? [String: AnyObject] {
				...
				user.userID = snapshot.key //ep.9 added so we know who we are sending to
	...
	override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		cell.profileImageView.loadImageUsingCacheWithUrlString(user.profilePic) //network still constantly firing downloads and downloading images for our view
