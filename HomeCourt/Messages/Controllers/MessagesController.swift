...
class MessagesController: UITableViewController {
	...
	override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) { //ep.12
		...
		ref.observeSingleEvent(of: .value, with: { (snapshot) in //ep.12
			...
			let user = CurrentUser(_dictionary: dictionary as NSDictionary) //ep.12
			user.userID = chatPartnerId //ep.12 set the toId or userUid as the chatPArtnerId
	...
	func setupNavBarWithCurrentUser(user: CurrentUser) { //ep.7
	...
	//showChatController
	func showChatControllerForUser(user: CurrentUser) { //ep.8 //ep.9 user parameter is added so we have a reference to which user we selected and want to send text to
